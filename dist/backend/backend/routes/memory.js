...
function memoryRoutes(memoryService) {
    ...
    router.post('/:scope/:scopeId/update', async (req, res) => {
        try {
            ...
            const memory = await memoryService.applyMemoryUpdate(scope, scopeId, req.body);
            res.json(memory);
        }
        catch (error) {
    ...
    router.get('/:scope/:scopeId/search', async (req, res) => {
        try {
            ...
            const results = await memoryService.searchMemory(scope, scopeId, q);
            res.json(results);
        }
        catch (error) {
    ...
    router.get('/:scope/:scopeId/stats', async (req, res) => {
        try {
            ...
            const stats = await memoryService.getMemoryStats(scope, scopeId);
            res.json(stats);
        }
        catch (error) {
    ...
    router.delete('/:scope/:scopeId', async (req, res) => {
        try {
            ...
            await memoryService.clearMemory(scope, scopeId);
    ...
    router.get('/:scope/:scopeId/export', async (req, res) => {
        try {
            ...
            const exported = await memoryService.exportMemory(scope, scopeId);
    ...
    router.post('/:scope/:scopeId/import', async (req, res) => {
        try {
            ...
            const memory = await memoryService.importMemory(scope, scopeId, JSON.stringify(req.body));
            res.json(memory);
        }
        catch (error) {
