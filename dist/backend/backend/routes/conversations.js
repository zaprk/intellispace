...
function conversationRoutes(prisma, conversationService) {
    const router = (0, express_1.Router)();
    // Get all conversations
    router.get('/', async (req, res) => {
        try {
            const { projectId } = req.query;
            const conversations = await conversationService.getConversations(projectId);
    ...
    router.get('/:id/messages', async (req, res) => {
        try {
            ...
            const messages = await conversationService.getMessages(req.params.id, parseInt(limit), parseInt(offset));
            res.json(messages);
        }
        catch (error) {
    ...
    router.get('/:id/search', async (req, res) => {
        try {
            ...
            const messages = await conversationService.searchMessages(req.params.id, q);
            res.json(messages);
        }
        catch (error) {
    ...
    router.get('/:id/export', async (req, res) => {
        try {
            ...
            const exported = await conversationService.exportConversation(req.params.id, format);
